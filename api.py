"""
═══════════════════════════════════════════════════════════════════════════════
FOGHORN API — FastAPI Endpoints for Nina Desktop
═══════════════════════════════════════════════════════════════════════════════

HTTP interface to Foghorn kernel.

Routes:
    /foghorn/cards          GET/POST    Card management
    /foghorn/cards/{id}     GET/PUT/DEL Single card operations
    /foghorn/query          POST        Search/query
    /foghorn/services       GET         List services
    /foghorn/services/run   POST        Execute service
    /foghorn/inspect/{id}   GET         Object inspector
    /foghorn/tinytalk       POST        Run TinyTalk code
    /foghorn/workspace      GET         Workspace stats
    /foghorn/undo           POST        Undo last action
    /foghorn/redo           POST        Redo undone action
...
router = APIRouter(prefix="/foghorn", tags=["Foghorn"])


def timed_response(func):
    """Decorator to add timing to response."""
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            return APIResponse(success=True, elapsed_us=elapsed_us, data=result)
...
def run_tinytalk(request: TinyTalkRequest):
    ...
    try:
        ...
        from realTinyTalk.foghorn_stdlib import register_foghorn_stdlib
        
        # Create runtime with Foghorn stdlib
        runtime = Runtime()
        register_foghorn_stdlib(runtime)
...
def mount_foghorn_api(app):
    """Mount Foghorn API to a FastAPI application."""
