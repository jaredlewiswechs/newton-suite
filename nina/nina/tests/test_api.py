...
from nina.api import router as nina_router
from nina.kernel import get_object_store
...
@pytest.fixture
def app():
    """Create FastAPI app with Nina routes."""
    _app = FastAPI()
    _app.include_router(nina_router)
    return _app
...
class TestCardAPI:
    ...
    def test_list_cards_empty(self, client):
        ...
        response = client.get("/nina/cards")
    ...
    def test_create_card(self, client):
        """Create a card."""
        response = client.post("/nina/cards", json={
            "title": "Test Card",
    ...
    def test_get_card(self, client):
        """Get a card by ID."""
        # Create first
        create_resp = client.post("/nina/cards", json={
            "title": "Retrievable",
            "content": "Find me"
        })
        ...
        # Get by ID
        response = client.get(f"/nina/cards/{card_id}")
        assert response.status_code == 200
    ...
    def test_update_card(self, client):
        ...
        # Update
        response = client.put(f"/nina/cards/{card_id}", json={
            "title": "Updated",
            "content": "New content"
        })
    ...
    def test_delete_card(self, client):
        ...
        # Delete
        response = client.delete(f"/nina/cards/{card_id}")
        assert response.status_code == 200
        assert response.json()["success"] == True
        
        # Verify gone
        get_resp = client.get(f"/nina/cards/{card_id}")
        assert get_resp.status_code == 404
    
    def test_list_cards_with_filter(self, client):
        """List cards filtered by tag."""
        # Create some cards
        client.post("/nina/cards", json={
            "title": "Tagged",
            "content": "Has the tag",
            "tags": ["special"]
        })
        ...
        # Filter by tag
        response = client.get("/nina/cards?tag=special")
        assert response.status_code == 200
...
class TestServiceAPI:
    ...
    def test_list_services(self, client):
        ...
        response = client.get("/nina/services")
        assert response.status_code == 200
    ...
    def test_run_echo_service(self, client):
        ...
        # Run service
        response = client.post("/nina/services/run", json={
            "service_name": "Echo",
...
class TestQueryAPI:
    ...
    def test_execute_query(self, client):
        ...
        response = client.post("/nina/query", json={
            "text": "What is Newton?"
        })
...
class TestInspectorAPI:
    ...
    def test_inspect_card(self, client):
        ...
        # Inspect
        response = client.get(f"/nina/inspect/{card_id}")
        assert response.status_code == 200
    ...
    def test_inspect_not_found(self, client):
        ...
        response = client.get("/nina/inspect/nonexistent")
        assert response.status_code == 404
...
class TestWorkspaceAPI:
    ...
    def test_workspace_stats_empty(self, client):
        ...
        response = client.get("/nina/workspace")
        assert response.status_code == 200
    ...
    def test_workspace_stats_with_objects(self, client):
        ...
        # Add some cards
        client.post("/nina/cards", json={"title": "A", "content": "1"})
        client.post("/nina/cards", json={"title": "B", "content": "2"})
    ...
    def test_undo_redo(self, client):
        ...
        undo_resp = client.post("/nina/undo")
        assert undo_resp.status_code == 200
        
        redo_resp = client.post("/nina/redo")
...
class TestLinkAPI:
    ...
    def test_create_link(self, client):
        ...
        # Create two cards
        card1 = client.post("/nina/cards", json={
            "title": "Source",
            "content": "From here"
        }).json()["data"]
        
        card2 = client.post("/nina/cards", json={
            "title": "Target",
            "content": "To here"
        }).json()["data"]
        
        # Create link
        response = client.post("/nina/links", json={
    ...
    def test_list_links(self, client):
        ...
        client.post("/nina/links", json={
            "source_hash": card1["hash"],
            "target_hash": card2["hash"]
        })
        
        # List
        response = client.get("/nina/links")
        assert response.status_code == 200
    ...
    def test_list_links_filtered(self, client):
        ...
        card3 = client.post("/nina/cards", json={
            "title": "C", "content": "3"
        }).json()["data"]
        ...
        # Filter by source
        response = client.get(f"/nina/links?source={card1['hash']}")
        assert len(response.json()["data"]) == 1
...
class TestTinyTalkAPI:
    ...
    def test_run_simple_code(self, client):
        ...
        response = client.post("/nina/tinytalk", json={
            "code": "2 + 3"
        })
